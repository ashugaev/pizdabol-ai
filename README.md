...
The bot distills a compact **author profile** — a short set of one-sentence facts about who you are. It is refreshed from **every diary message you send** (voice or text), best-effort in the background so it never delays the preview. The profile captures durable, decision-shaping context: long-term personality traits and biases, values, recurring patterns and drivers, key relationships and goals, and the current life phase you are going through (medium-term, not day-to-day). New knowledge is merged into the existing profile and semantically deduped, keeping only lean, durable facts; transient one-offs (what you ate, bodily events, a single moment's mood, a plain retelling of the day) are dropped, and if a message adds nothing new the list is left unchanged. The points persist in local state and are fed back as background context on the next roast so the model keeps getting to know you. The list is capped and kept terse. Use `OPENAI_PROFILE_MODEL` (defaults to `OPENAI_SUMMARY_MODEL`) to pick the model that maintains it.
