...
The preview shows a **🔥 Roast** button next to **Save**. Pressing it sends the current draft text to a high-reasoning OpenAI model that plays a blunt-but-caring street-bro («разъёб»): honest, on your side, teasing where it helps, never sugar-coating. The analysis is posted as a **new reply** in the chat. The original draft is never modified — Save still writes exactly what's in the preview.
...
The bot distills a compact **author profile** — a short set of one-sentence facts about who you are (values, habits, relationships, recurring patterns). It is refreshed from **every diary message you send** (voice or text), best-effort in the background so it never delays the preview. New knowledge is merged into the existing profile and semantically deduped, keeping only lean, durable facts; transient one-offs (what you ate, bodily events, a single moment's mood, a plain retelling of the day) are dropped, and if a message adds nothing new the list is left unchanged. The points persist in local state and are fed back as background context on the next roast so the model keeps getting to know you. The list is capped and kept terse. Use `OPENAI_PROFILE_MODEL` (defaults to `OPENAI_SUMMARY_MODEL`) to pick the model that maintains it.

To seed the profile from your existing Notion history, run the backfill script: `python -m scripts.backfill_profile` (or `make backfill-profile`). It reads **all** diary pages (oldest first), batches their text through the same extraction, and persists incrementally. Flags: `--dry-run` (extract without persisting), `--reset` (start from an empty profile instead of merging), `--batch-size N`, `--max-chars N`. It talks to Notion and OpenAI, so it needs real credentials in `.env`.
...
│   └── roast.py            # «Разъёб» mode via OpenAI (high reasoning)
