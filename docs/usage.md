...
There are two ways to change the list. Tell the bot in a roast reply how it should behave — or that it should forget a rule — and it rewrites the list itself. This works **in any turn**: the first roast, a follow-up, a voice reply. The model appends a compact delta (added, removed, reworded rules) to the end of its answer behind an internal marker; the bot strips the marker, merges the delta locally, and posts a short `🧠 Rules updated` note. When nothing should change the model sends no delta at all, so a normal reply costs nothing extra and **local state is never rewritten**. An unreadable delta is dropped and changes nothing.
