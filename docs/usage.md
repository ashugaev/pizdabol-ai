...
`/rules` prints the current numbered list. Rules persist in local state and are fed into every roast, and every saved change is mirrored to a **`Memory — Bot rules`** page next to your diary database — see [Configuration](configuration.md#memory-pages).
...
The profile is also mirrored to a **`Memory — Author profile`** page sitting next to your diary database, so you can read what the bot knows in the same place as your notes — see [Configuration](configuration.md#memory-pages).
